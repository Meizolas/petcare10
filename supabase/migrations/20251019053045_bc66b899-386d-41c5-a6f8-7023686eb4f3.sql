-- Fix Critical Privilege Escalation Risk
-- Add restrictive RLS policies to user_roles table to prevent privilege escalation

-- Only allow role insertion via trigger on user signup (block all direct inserts)
CREATE POLICY "Roles can only be assigned by system"
ON public.user_roles FOR INSERT
WITH CHECK (false);

-- Prevent role modifications
CREATE POLICY "Roles cannot be modified"
ON public.user_roles FOR UPDATE
USING (false);

-- Prevent role deletions by users
CREATE POLICY "Roles cannot be deleted by users"
ON public.user_roles FOR DELETE
USING (false);

-- Fix Webhook URL Validation Risk
-- Add validation function to prevent SSRF attacks and ensure only HTTPS URLs

CREATE OR REPLACE FUNCTION public.validate_webhook_url()
RETURNS TRIGGER AS $$
BEGIN
  -- Only allow HTTPS
  IF NEW.webhook_url !~ '^https://[^/]+' THEN
    RAISE EXCEPTION 'Webhook URL must use HTTPS protocol';
  END IF;
  
  -- Block localhost and internal IPs
  IF NEW.webhook_url ~* '(localhost|127\.0\.0\.1|169\.254\.)' THEN
    RAISE EXCEPTION 'Webhook URL cannot target internal networks';
  END IF;
  
  -- Block private IP ranges (10.x.x.x, 172.16-31.x.x, 192.168.x.x)
  IF NEW.webhook_url ~* '(10\.|172\.(1[6-9]|2[0-9]|3[01])\.|192\.168\.)' THEN
    RAISE EXCEPTION 'Webhook URL cannot target private networks';
  END IF;
  
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER validate_webhook_url_trigger
  BEFORE INSERT OR UPDATE ON public.webhook_config
  FOR EACH ROW
  EXECUTE FUNCTION public.validate_webhook_url();